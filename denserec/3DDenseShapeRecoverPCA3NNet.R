...
pcx=as.matrix(pcX)[,1:2]
pcy=as.matrix(pcY)[,1:2]
...
nn<-mlp(pcx,pcy,size=5)
pctestx<-predict(prX,test)[1:2]
pctesty<-predict(nn,pctestx)

eigY=t(prY$rotation)
predict=pctesty[1]*eigY[1,]+pctesty[2]*eigY[2,]+prY$center
pred=predict
